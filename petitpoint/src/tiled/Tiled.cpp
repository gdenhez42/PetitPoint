...
namespace pp::tiled
{
  ...
  bool TileSet::Init(const std::string& p_filename)
  {
    ...
    if (success) {
      ...
      m_count = root->IntAttribute(TILE_COUNT);
  ...
  bool TileMap::Init(const std::string& p_name)
  {
    ...
    if (success) {
      ...
      while (elem != nullptr) {
        ...
        if (TILESET == elemName) {
          EmbeddedTileset tileset;
          tileset.m_firstGid = elem->IntAttribute(FIRST_GID);
          tileset.m_source = GetStringAttribute(elem, SOURCE);
          if (tileset.m_source == EMPTY) {
            tileset.m_isRef = false;
            tileset.m_name = GetStringAttribute(elem, NAME);
            tileset.m_width = elem->IntAttribute(TILE_WIDTH);
            tileset.m_height = elem->IntAttribute(TILE_HEIGHT);
            tileset.m_count = elem->IntAttribute(TILE_COUNT);
            tileset.m_columns = elem->IntAttribute(COLUMNS);
          }
  ...
  const EmbeddedTileset& TileMap::FindTilesetRef(int p_Gid) const {
    std::vector<EmbeddedTileset>::const_iterator end = m_tilesets.end();
    std::vector<EmbeddedTileset>::const_iterator it = m_tilesets.begin();
    std::vector<EmbeddedTileset>::const_iterator found = m_tilesets.end();
    int max = 0;
    for(; it != end; ++it) {
